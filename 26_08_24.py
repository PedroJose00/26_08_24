...
fem_function_densidad_aparente = fem.Function(W,name="aparente")
...
with XDMFFile(mesh.comm, "./26_08_24/fem_function_modulo_E0.xdmf", "w") as file:
          file.write_mesh(mesh)
          file.write_function(fem_function_modulo_E_0 ,0.0)

with XDMFFile(mesh.comm, "./26_08_24/fem_function_modulo_rho.xdmf", "w") as file:
          file.write_mesh(mesh)
          file.write_function(fem_function_densidad_aparente ,0.0)
          
# -------------------------------------------------------------------------------
# Impresion de modulo de elasticidad y densidad aparente OK -- 22-08-24
# Se realiza lectura y calculo del modulo a partir del valor de densidad
# -------------------------------------------------------------------------------

# # Definimos la función f(alpha) según la ecuación dada
# def f(alpha, array_modulo_E_0, array_densidad_gr_cm3):
#     return np.log(array_modulo_E_0) - np.log(84370) - 2.58 * np.log(array_densidad_gr_cm3) - 2.74 * np.log(alpha) + 2.58 * np.log(1.41 + 1.29 * alpha)

# # Definimos la derivada de f(alpha)
# def f_prime(alpha, array_modulo_E_0, array_densidad_gr_cm3):
#     return -2.74 / alpha + (2.58 * 1.29) / (1.41 + 1.29 * alpha)

# # Inicializamos un arreglo para almacenar las soluciones de alpha
# alpha_solutions = np.zeros_like(array_modulo_E_0)

# tolerancia = 1e-6
# max_iteraciones = 100

# # Limite en el crecimiento de alpha por iteración
# limite_cambio_alpha = 0.1

# for i in range(len(array_modulo_E_0)):
#     E = array_modulo_E_0[i]
#     rho_a = array_densidad_gr_cm3[i]
    
#     # Ajustar el valor inicial, comenzando desde un valor pequeño
#     alpha_initial_guess = 0.01
    
#     try:
#         # Aplicar Newton-Raphson manualmente con limitación de crecimiento
#         alpha_actual = alpha_initial_guess
#         for _ in range(max_iteraciones):
#             f_val = f(alpha_actual, E, rho_a)
#             f_prime_val = f_prime(alpha_actual, E, rho_a)
            
#             if f_prime_val == 0:
#                 raise RuntimeError("Derivada cero encontrada.")
            
#             alpha_nuevo = alpha_actual - f_val / f_prime_val
            
#             # Limitar el cambio en alpha por iteración
#             if np.abs(alpha_nuevo - alpha_actual) > limite_cambio_alpha:
#                 alpha_nuevo = alpha_actual + np.sign(alpha_nuevo - alpha_actual) * limite_cambio_alpha
            
#             if np.abs(alpha_nuevo - alpha_actual) < tolerancia:
#                 alpha_actual = alpha_nuevo
#                 break
            
#             alpha_actual = alpha_nuevo
        
#         # Limitar alpha al máximo permitido
#         if alpha_actual > 0.6:
#             alpha_actual = 0.6
        
#         alpha_solutions[i] = alpha_actual
    
#     except RuntimeError as e:
#         print(f"Error en la iteración {i}: {e}")
#         alpha_solutions[i] = np.nan  # O algún otro valor por defecto


# fem_function_alpha.x.array[:] = alpha_solutions

# with XDMFFile(mesh.comm, "./26_08_24/fem_function_alpha.xdmf", "w") as file:
#           file.write_mesh(mesh)
#           file.write_function(fem_function_alpha ,0.0)
...
# ----------------------------------------------------------------------------------
# with XDMFFile(mesh.comm, "./26_08_24/fem_function_modulo_E_0.xdmf", "w") as file:
#           file.write_mesh(mesh)
#           file.write_function(fem_function_modulo_E_f ,0.0)


# with XDMFFile(mesh.comm, "./26_08_24/fem_function_densidad_0.xdmf", "w") as file:
#           file.write_mesh(mesh)
...
nu_values = np.zeros_like(array_BVTV)
epsilon_xx = np.zeros_like(array_BVTV)
epsilon_yy = np.zeros_like(array_BVTV)
epsilon_zz = np.zeros_like(array_BVTV)
array_sigma_xx = np.zeros_like(array_BVTV)
array_sigma_yy = np.zeros_like(array_BVTV)
array_sigma_zz = np.zeros_like(array_BVTV)

# Exportar resultados
with XDMFFile(MPI.COMM_WORLD, "./Uh_step_00.xdmf", "w") as xdmf:
    xdmf.write_mesh(mesh)
    xdmf.write_function(uh)
    
# Calcular las deformaciones en todos los puntos de la malla
V_epsilon = fem.FunctionSpace(mesh, ("DG", 0))
fem_epsilon_xx = fem.Function(V_epsilon, name="epsilon_xx")
fem_epsilon_yy = fem.Function(V_epsilon, name="epsilon_yy")
fem_epsilon_zz = fem.Function(V_epsilon, name="epsilon_zz")

sigma_xx = Function(V_epsilon, name="sigma_xx")
sigma_yy = Function(V_epsilon, name="sigma_yy")
sigma_zz = Function(V_epsilon, name="sigma_zz")

# Evaluar y asignar manualmente las componentes del tensor de deformación
eps = epsilon(uh)
tensor_sigma = sigma(uh)
fem_epsilon_xx.interpolate(fem.Expression(eps[0, 0], V_epsilon.element.interpolation_points()))
fem_epsilon_yy.interpolate(fem.Expression(eps[1, 1], V_epsilon.element.interpolation_points()))
fem_epsilon_zz.interpolate(fem.Expression(eps[2, 2], V_epsilon.element.interpolation_points()))

expr_sig_xx = fem.Expression(tensor_sigma[0, 0], V_epsilon.element.interpolation_points())
expr_sig_yy = fem.Expression(tensor_sigma[1, 1], V_epsilon.element.interpolation_points())
expr_sig_zz = fem.Expression(tensor_sigma[2, 2], V_epsilon.element.interpolation_points())


epsilon_xx = fem_epsilon_xx.x.array[:]
sigma_xx.interpolate(expr_sig_xx)
sigma_yy.interpolate(expr_sig_yy)
sigma_zz.interpolate(expr_sig_zz)

array_sigma_xx = sigma_xx.x.array[:]
array_sigma_yy = sigma_yy.x.array[:]
array_sigma_zz = sigma_zz.x.array[:]

# Exportar las deformaciones a archivos XDMF
with XDMFFile(mesh.comm, "strain_xx.xdmf", "w") as xdmf:
    xdmf.write_mesh(mesh)
    xdmf.write_function(fem_epsilon_xx)

with XDMFFile(mesh.comm, "strain_yy.xdmf", "w") as xdmf:
    xdmf.write_mesh(mesh)
    xdmf.write_function(fem_epsilon_yy)

with XDMFFile(mesh.comm, "strain_zz.xdmf", "w") as xdmf:
    xdmf.write_mesh(mesh)
    xdmf.write_function(fem_epsilon_zz)



# Crear espacio de funciones para el coeficiente de Poisson
V_nu = fem.FunctionSpace(mesh, ("DG", 0))
fem_nu_u = fem.Function(V_nu, name="Poisson_ratio")

# Calcular el coeficiente de Poisson en cada punto de la malla
#nu_values = np.empty_like(array_BVTV)

# print("NaN en sigma_xx:", np.isnan(array_sigma_xx).any())
# print("Inf en sigma_xx:", np.isinf(array_sigma_xx).any())
# print("NaN en sigma_yy:", np.isnan(array_sigma_yy).any())
# print("Inf en sigma_yy:", np.isinf(array_sigma_yy).any())
# print("NaN en sigma_zz:", np.isnan(array_sigma_zz).any())
# print("Inf en sigma_zz:", np.isinf(array_sigma_zz).any())

# # Filtrar NaNs e Infs antes de calcular el promedio
# filtered_sigma_xx = array_sigma_xx[np.isfinite(array_sigma_xx)]
# filtered_sigma_yy = array_sigma_yy[np.isfinite(array_sigma_yy)]
# filtered_sigma_zz = array_sigma_zz[np.isfinite(array_sigma_zz)]
# filtered_epsilon_xx = epsilon_xx[np.isfinite(epsilon_xx)]

# promedio_sigma_xx = np.mean(filtered_sigma_xx)
# promedio_sigma_yy = np.mean(filtered_sigma_yy)
# promedio_sigma_zz = np.mean(filtered_sigma_zz)
# promedio_epsilon_xx = np.mean(filtered_epsilon_xx)
# promedio_E_f = np.mean(array_modulo_E_f)  # Asegúrate de que array_modulo_E_f esté definido y no contenga NaNs o Infs

# print(f"Promedio de sigma_xx: {promedio_sigma_xx}")
# print(f"Promedio de sigma_yy: {promedio_sigma_yy}")
# print(f"Promedio de sigma_zz: {promedio_sigma_zz}")
# print(f"Promedio de epsilon_xx: {promedio_epsilon_xx}")
# print(f"Promedio de E_f: {promedio_E_f}")



# promedio_sigma_xx = np.mean(sigma_xx)
# promedio_sigma_yy = np.mean(sigma_yy)
# promedio_sigma_zz = np.mean(sigma_zz)
# promedio_epsilon_xx = np.mean(epsilon_xx)
# promedio_E_f = np.mean(array_modulo_E_f)


print("El valor promedio de la E_f es:", promedio_E_f)

nu_values = (array_sigma_xx -array_modulo_E_f * epsilon_xx)/(array_sigma_yy + array_sigma_zz)


fem_nu_u.x.array[:]=nu_values
fem_nu_u.x.scatter_forward()
# Exportar el coeficiente de Poisson
with XDMFFile(MPI.COMM_WORLD, "poisson_A_ratio.xdmf", "w") as xdmf:
    xdmf.write_mesh(mesh)
    xdmf.write_function(fem_nu_u)
...
with XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_modulo.xdmf", "w") as file_modulo, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_BVTV.xdmf", "w") as file_BVTV, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_vm.xdmf", "w") as file_array_vm, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_damage.xdmf", "w") as file_array_damage, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_alpha.xdmf", "w") as file_alpha, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi.xdmf", "w") as file_array_psi, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_error.xdmf", "w") as file_array_error, \
     XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_densidad_fase_1.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_2/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_3/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_4/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
    ...
    with XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_5/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_51/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
    ...
    with XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_6/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#-------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_7/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
    ...
    with XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./26_08_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./26_08_24/fase_71/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
data.to_csv('./26_08_24/resultados_promedio_final.csv', index=False)
